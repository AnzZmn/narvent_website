...
import { CLOSING, INTRO, LAST_UPDATED, SECTIONS } from "./terms-content";

const MONO = "font-[family-name:var(--font-mono)]";
const GUTTER = "px-5 sm:px-8 lg:px-[clamp(32px,5vw,88px)]";

export default function NarventTerms() {
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-[#0e0719]">
      ...
      >
        ...
        <main className="flex min-w-0 flex-col gap-10 pb-10 lg:gap-14">
          {SECTIONS.map((s, i) => (
            <TermsSection key={s.title} section={s} index={i} />
          ))}
            ...
            <div className="mt-5 flex flex-wrap gap-x-14 gap-y-6">
              <div>
                ...
                <a
                  ...
                  className="mt-2 flex min-h-11 items-center text-[17px] font-semibold tracking-[-0.02em] text-[var(--color-1)] hover:underline lg:text-[19px]"
                >
                  narvent.in
                </a>
              </div>
              <div>
                ...
                <a
                  href="mailto:legal@narvent.in"
                  className="mt-2 flex min-h-11 items-center break-all text-[17px] font-semibold tracking-[-0.02em] text-[var(--color-1)] hover:underline lg:text-[19px]"
                >
                  legal@narvent.in
                </a>
              </div>
